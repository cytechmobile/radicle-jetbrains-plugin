...
import network.radicle.jetbrains.radiclejetbrainsplugin.patches.overview.PatchEditorComponent;
...
public class OverviewTest extends AbstractIT {
    ...
    private PatchEditorComponent patchEditorComponent;
    private RadPatch patch;

    @Before
    public void beforeTest() throws VcsException {
        patch = createPatch();
        var gitRepoManager = GitRepositoryManager.getInstance(getProject());
        var repos = gitRepoManager.getRepositories();
        patch.repo = repos.get(0);
        patchEditorComponent = new PatchEditorComponent(patch);
    ...
    @Test
    public void testCommentsSection() {
        var commentSection = patchEditorComponent.getComponentsFactory().getCommentSection();
        var elements = findElement((JPanel) commentSection, new BaseHtmlEditorPane(), new ArrayList<>());
        var timeline = "";
        for (var el : elements) {
            timeline += ((BaseHtmlEditorPane) el).getText();
        }
        assertThat(timeline).contains(patch.revisions.get(0).discussions().get(0).author().id());
        assertThat(timeline).contains(patch.revisions.get(0).discussions().get(0).body());

        assertThat(timeline).contains(patch.revisions.get(1).discussions().get(0).author().id());
        assertThat(timeline).contains(patch.revisions.get(1).discussions().get(0).body());
    }
