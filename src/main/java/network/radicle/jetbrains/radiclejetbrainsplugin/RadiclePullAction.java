    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        ApplicationManager.getApplication().executeOnPooledThread(() -> {
            for (var repo : repos) {
                ...
                if (!success) {
                    logger.warn("error in rad pull: exit:{}, out:{} err:{}", output.getExitCode(), output.getStdout(), output.getStderr());
                    RadicleSyncAction.showErrorNotification(repo.getProject(), "radCliError", output.getStderr());
                    return;
                }
                logger.info("success in rad pull: exit:{}, out:{} err:{}", output.getExitCode(), output.getStdout(), output.getStderr());
                RadicleSyncAction.showNotification(project, "", "Remotes fetched", NotificationType.INFORMATION, null);
            }
        });
    }
}
